"""Goal-conditioned 3D GNCA with the original Boids GNCA structure."""
...
class GoalConditionedGNNCABoids3D(tf.keras.Model):
    """Evolve physical state from ``[position, velocity, relative_goal]``.

    Each node receives nine features: 3D position, 3D velocity, and the
    relative vector from its position to the active goal.  The model predicts
    only the next six-dimensional physical state, exactly as the conditioned
    2D model predicts only its four-dimensional physical state.
    """
    ...
    def build(self, input_shape):
        ...
        self.mp_diff = SimpleEdgeConv(
            3, activation="linear", mlp_hidden=[self.hidden]
        )
    ...
    def call(self, inputs, training=False):
        ...
        position = conditioned_state[:, :3]
        velocity = conditioned_state[:, 3:6]
        ...
        relative_effect = tf.cast(relative_effect, velocity.dtype)
        absolute_effect = tf.cast(absolute_effect, velocity.dtype)
